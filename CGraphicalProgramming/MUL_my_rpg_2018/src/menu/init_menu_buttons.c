...
buttons_t   *init_menu_buttons_pt_two(buttons_t *new)
{
    sfVector2f pos = init_vector_two_f(410, 800);
...
buttons_t   *init_menu_buttons(void)
{
    ...
    sfVector2f  pos = init_vector_two_f(410, 600);
    sfIntRect   rect = init_int_rect(0, 202, 0, 90);
    ...
    pos = init_vector_two_f(410, 700);
    new[1].button_text = init_texture_struc("assets/Tutorial.png");
    sfSprite_setPosition(new[1].button_text->sprite, pos);
    new[1].button_anim = init_anim_struc(pos, rect, -1, -1);
    new = init_menu_buttons_pt_two(new);
    return (new);
}