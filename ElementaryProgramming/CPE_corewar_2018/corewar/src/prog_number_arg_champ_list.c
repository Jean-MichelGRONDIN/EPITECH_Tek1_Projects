...
int ac, char **av, int *index)
{
    ...
    if (my_str_is_int(av[after]) == 0) {
        my_pstrerr("Programe number must be an integer !\n");
        *index = -1;
...
int ac, char **av, int *index)
{
    ...
    *index = *index + 2;
    res = get_prog_adress_arg_champ_list_value(list, ac, av, index);
    if (res != -1 && *index != -1)
        *index = *index + 2;
    else if (*index == -1)
    ...
    list = add_to_arg_champ_list(prog, my_getnbr(av[after]), res, list);
    *index = *index + 1;
    return (list);
}
...
int ac, char **av, int *index)
{
    ...
    if (after >= ac) {
        my_pstrerr("No info for prog number");
        *index = -1;
        list = free_arg_champ_list(list);
        return (list);
    ...
    return (prog_number_pt_two(list, ac, av, index));
}